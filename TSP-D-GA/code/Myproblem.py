...
class MyProblem:
    ...
    def neighborhoods1(self, idvi, operator):

        if operator == '2opt':
            # [solution_current, value_current, count] = self.n_2opt1(pop, value, idvi)
            # [solution_current] = self.n_2opt1(pop, idvi)
            solution_current = self.n_2opt1(idvi)

        elif operator == 'swap':
            solution_current = self.n_swap1(idvi)

        elif operator == 'relocate':
            solution_current = self.n_relocate1(idvi)

        elif operator == '3opt':
            # [solution_current, value_current, count] = self.n_3opt1(pop, value, solution, idvi)
            solution_current = self.n_3opt1(idvi)

        elif operator == 'swap2':
            solution_current = self.n_swap2_1(idvi)
    ...
    def n_3opt1(self, p):
        ...
        route = route[:i + 1] + route[j + 1:k + 1] + route[i + 1:j + 1] + route[k + 1:]
    ...
    def n_relocate1(self, p):
        route = copy.deepcopy(p)  # 第一层染色体
        n = len(route)
        ind = random.randint(1, n - 2)  # 生成一个不是首元素的随机索引
        remove_node = route[ind]
        route = [x for x in route if x != remove_node]  # 删去这个节点
        insert = random.randint(1, n - 2)
        while insert == ind:  # 如果插入的位置和原来一样了，就重新生成插入位置
            insert = random.randint(1, n - 2)
        route.insert(insert, remove_node)  # 重新插入新位置
